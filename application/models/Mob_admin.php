...
class Mob_admin extends CI_Model
{
    ...
    function checkPromo($insta_id, $promo, $package_type = '')
    {
        ...
        $sql = 'SELECT `p`.`promo_code`, `p`.`percentage_discount` as discountPercent, p.promo_category, p.start_date , p.end_date, l.promo_limit,
                (SELECT count(insta_id) FROM promo_details WHERE promo_code = p.promo_code AND insta_id = "' . $insta_id . '") as promo_used 
                FROM promo p 
                LEFT JOIN promo_limit l on l.promo_id = p.id
                WHERE p.promo_code = "' . $promo . '" AND l.promo_used = 0';
        if ($package_type != '') {
            $sql .= ' AND (p.promo_category = "' . $package_type . '" || p.promo_category = "all") ';
        }
    ...
    function getOrderItemByOrderId($order_id)
    {
        ...
        cart_order_selected_post.post_id, cart_order_selected_post.comments,cart_order_selected_post.api_order_id, cart_order_selected_post.id as cart_order_selected_id');
